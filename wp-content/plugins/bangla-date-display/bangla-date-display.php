...
function bddp_bangla_time() {
	...
	if ($hour >= 5 && $hour <= 5) { $ddd = "ভোর "; }
	else if ($hour >= 6 && $hour <= 11) { $ddd = "সকাল "; }
	else if ($hour >= 12 && $hour <= 14) { $ddd = "দুপুর "; }
	else if ($hour >= 15 && $hour <= 17) { $ddd = "বিকাল "; }
	else if ($hour >= 18 && $hour <= 19) { $ddd = "সন্ধ্যা "; }
	else { $ddd = "রাত "; }
...
function bddp_bn_day() {
	...
	$str = en_to_bn(gmdate("l", time()+$bddp_options['bangla_tz']*60*60));
	return $str;
	}
...
function bddp_bangla_date() {
	...
	if ( $bddp_options['ord_suffix'] == "1" ) { printf('%s', $day_number[$day] . ' ' . $month_year . $last_word); }
...
function bddp_bn_season() {
	...
	if($month == "বৈশাখ" || $month == "জ্যৈষ্ঠ") { return "গ্রীষ্মকাল"; }
	elseif($month == "আষাঢ়" || $month == "শ্রাবণ") { return "বর্ষাকাল"; }
	elseif($month == "ভাদ্র" || $month == "আশ্বিন") { return "শরৎকাল"; }
	elseif($month == "কার্তিক" || $month == "অগ্রহায়ণ") { return "হেমন্তকাল"; }
	elseif($month == "পৌষ" || $month == "মাঘ") { return "শীতকাল"; }
	else { return "বসন্তকাল"; }
}
